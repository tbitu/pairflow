...
const metaReviewSubmitFailureRecoveryChecklist =
  "Meta-review submit failure recovery: Invalid --report-json -> rebuild as valid JSON object with double-quoted keys/strings; CLAIM_SOURCE_INVALID -> set findings_claim_source=meta_review_artifact whenever findings_claim_state is present; findings_count required/invalid -> set findings_count to a non-negative integer and keep claim/count tuple consistent (open_findings => findings_count>0, clean => findings_count=0); META_REVIEW_FINDINGS_PARITY_GUARD digest mismatch -> do not embed a digest inside the findings JSON file, compute sha256 of the static file on disk and pass it in findings_digest_sha256; META_REVIEW_GATE_REVIEWER_CONVERGENCE_CONFLICT with snapshot totals -> copy snapshot_open_total/snapshot_advisory into findings_count/findings_claimed_open_total/findings_advisory_open_total and keep findings_blocking_open_total=0 for advisory-only approve.";
